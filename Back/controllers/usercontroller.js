...
const ERROR_MESSAGES = {
  ...
  const signup = async (req, res, next) => {
    const { nom, prenom, mail, numero,age,sex,weight,chronicDisease,ecgSignal, password,role } = req.body;
...
const forget = async (req, res, next) => {
  ...
  const getUserbyNom = async (req, res,next) => {
    const user = await UserModel.findOne({
      nom:req.params.nom
    }).then((user) => {
      res.status(200).json(user);
  ...
  const updateUser = async (req, res) => {
    ...
    module.exports = {getAllUsers,getUserbyNom, logOut,signup,login,forget,reset, showUsers,getUser, deleteUser, updateUser, findUser };
